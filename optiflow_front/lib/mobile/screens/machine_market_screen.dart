                ...
                itemBuilder: (_, index) {
                  ...
                  return GestureDetector(
                    onTap: () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => MachineBookingPage(machine: machine),
                      ),
                    ),
                    child: Container(
                          ...
                          Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  'Rs. ${machine.pricePerHour.toStringAsFixed(0)}/hr',
                                ),
                              ],
                            ),
                          ),
...
class _MachineBookingPageState extends State<MachineBookingPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_machine == null) {
        throw Exception('This QR code is not a bookable machine.');
      }
      final availability = await ApiService.instance.fetchMachineAvailability(
        _machine!.id,
        _day,
      );
      if (mounted) {
        setState(() {
          _busy = List<Map<String, dynamic>>.from(
            availability['busy'] as List? ?? const [],
          );
          _loading = false;
          _error = null;
        });
  ...
  Future<void> _request() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Request sent for manager approval · Quote Rs. ${quote.toStringAsFixed(0)}',
          ),
        ),
      );
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : ListView(
              ...
              children: [
                Text(
                  'Rs. ${_machine!.pricePerHour.toStringAsFixed(0)}/hour',
                  style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.w800,
                  ),
                ...
                if (_busy.isEmpty)
                  const Text(
                    'No approved bookings or production tasks on this day.',
                  )
                else
                  ..._busy.map((row) {
                    final start = DateTime.tryParse(
                      (row['start_time'] ?? row['scheduled_start_time'])
                          .toString(),
                    )?.toLocal();
                    final end = DateTime.tryParse(
                      (row['end_time'] ?? row['scheduled_end_time']).toString(),
                    )?.toLocal();
                    return ListTile(
                      contentPadding: EdgeInsets.zero,
                      leading: const Icon(Icons.block_outlined),
                      title: Text(
                        '${start == null ? '?' : DateFormat('h:mm a').format(start)} – ${end == null ? '?' : DateFormat('h:mm a').format(end)}',
                      ),
                ...
                ListTile(
                  ...
                  subtitle: Text(
                    _start == null
                        ? 'Choose time'
                        : DateFormat('MMM d, h:mm a').format(_start!),
                  ),
                ...
                ListTile(
                  ...
                  subtitle: Text(
                    _end == null
                        ? 'Choose time'
                        : DateFormat('MMM d, h:mm a').format(_end!),
                  ),
                  trailing: const Icon(Icons.chevron_right),
                  onTap: () async {
                ...
                const SizedBox(height: 8),
                const Text(
                  'The machine is reserved only after manager approval.',
