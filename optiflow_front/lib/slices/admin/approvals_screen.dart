...
class _ApprovalsScreenState extends State<ApprovalsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await ApiService.instance.fetchBookingRequests(
        status: 'PENDING',
      );
      if (mounted) {
        setState(() {
    ...
    return ListView(
      ...
      children: [
          ...
          children: [
            const Column(
              ...
              children: [
                ...
                Text(
                  'External machine bookings become scheduler blockers only after approval.',
        ...
        if (_requests.isEmpty)
          const Text(
            'No pending machine booking requests.',
            style: TextStyle(color: AppColors.textSecondary),
          )
        else
          ..._requests.map((request) {
            final start = DateTime.tryParse(
              request['start_time']?.toString() ?? '',
            )?.toLocal();
            final end = DateTime.tryParse(
              request['end_time']?.toString() ?? '',
            )?.toLocal();
            return Card(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            request['machine_name']?.toString() ?? 'Machine',
                            style: const TextStyle(fontWeight: FontWeight.w800),
                          ),
                          const SizedBox(height: 4),
                          Text(
                            '${request['requested_by_name'] ?? 'External user'} · Quote Rs. ${request['quoted_amount'] ?? 0}',
                          ),
                    ...
                    TextButton(
                      onPressed: () => _decide(request, 'REJECTED'),
