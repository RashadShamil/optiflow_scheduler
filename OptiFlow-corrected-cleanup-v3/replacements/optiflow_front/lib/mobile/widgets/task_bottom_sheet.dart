...
class _TaskBottomSheetState extends State<TaskBottomSheet> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(task.jobTitle, style: const TextStyle(fontSize: 16, color: AppColors.textSecondary)),
          const SizedBox(height: 24),
          _row(Icons.flag_outlined, 'Priority', task.priority),
          _row(Icons.category_outlined, 'Operation', task.operationName),
          _row(Icons.inventory_2_outlined, 'Quantity', '${task.quantityToProcess} units'),
          _row(Icons.schedule_rounded, 'Scheduled', '${task.formattedDate} ${task.formattedTime}'.trim()),
          const SizedBox(height: 28),
          SizedBox(width: double.infinity, height: 56, child: _actionButton(task)),
        ],
      ),
    );
  }
  ...
  Widget _actionButton(TaskModel task) {
    ...
    return const Center(
      child: Text(
        'Completed',
        style: TextStyle(color: AppColors.completed, fontWeight: FontWeight.w800, fontSize: 16),
      ),
    );
  }
