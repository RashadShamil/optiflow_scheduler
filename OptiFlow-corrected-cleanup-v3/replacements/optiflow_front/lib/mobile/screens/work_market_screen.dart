...
class _WorkMarketScreenState extends State<WorkMarketScreen> {
  ...
  Future<void> _claim(WorkOfferModel offer) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Work claimed. The manager must re-optimize the job before it receives a time slot.'),
        ),
      );
      await _load();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.background,
        title: const Text('Work Market', style: TextStyle(fontWeight: FontWeight.w900)),
  ...
  Widget _body() {
    ...
    if (_offers.isEmpty && _mine.isEmpty) {
      return ListView(
        ...
        children: const [
          SizedBox(height: 120),
          Icon(Icons.work_off_outlined, size: 64, color: AppColors.textDisabled),
          SizedBox(height: 16),
    ...
    if (_mine.isNotEmpty) {
      cards.add(const Padding(
        ...
        child: Text('CLAIMED BY ME', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.textDisabled)),
      ));
      cards.addAll(_mine.map((offer) => Card(
        child: ListTile(
          title: Text(offer.title),
          subtitle: Text('${offer.jobTitle} · awaiting/using production schedule'),
    ...
    if (_offers.isNotEmpty) {
      cards.add(const Padding(
        padding: EdgeInsets.only(bottom: 10),
        child: Text('AVAILABLE', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.textDisabled)),
      ));
    }
    cards.addAll(_offers.map((offer) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text(offer.title, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text(offer.jobTitle, style: const TextStyle(color: AppColors.textSecondary)),
              ...
              Wrap(
                ...
                children: [
                  ...
                  Text('Rs. ${offer.payAmount.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.w900)),
                ],
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _claiming.contains(offer.id) ? null : () => _claim(offer),
