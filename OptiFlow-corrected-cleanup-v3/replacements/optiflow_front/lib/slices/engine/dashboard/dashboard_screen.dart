...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget _dashboard() {
    ...
    final cards = [
      ...
      ('Pending Tasks', _stats['pending_tasks'] ?? 0, Icons.pending_actions_rounded),
      ('Active Machines', _stats['active_machines'] ?? 0, Icons.precision_manufacturing_rounded),
      ('Offline Machines', offline, Icons.warning_amber_rounded),
      ('Booking Approvals', _stats['pending_booking_approvals'] ?? 0, Icons.approval_rounded),
      ('Open External Work', _stats['open_external_work_offers'] ?? 0, Icons.work_outline_rounded),
    ];
      ...
      child: ListView(
        ...
        children: [
          Row(
            ...
            children: [
              const Column(
                ...
                children: [
                  Text('Dashboard', style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900)),
                  SizedBox(height: 6),
                  Text('Live print-shop operations', style: TextStyle(color: AppColors.textSecondary)),
                ],
              ),
          ...
          Wrap(
            spacing: 16,
            runSpacing: 16,
            children: cards.map((card) => _metricCard(card.$1, card.$2, card.$3)).toList(),
          ),
          const SizedBox(height: 28),
          _alerts(),
          const SizedBox(height: 28),
  ...
  Widget _alerts() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Attention Needed', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800)),
          const SizedBox(height: 12),
          if (overdue.isEmpty && offline.isEmpty)
            const Text('No active alerts.', style: TextStyle(color: AppColors.success))
          else ...[
            ...offline.map((item) => _alertRow(Icons.power_off_rounded, '${item['name']} is ${item['status']}')),
            ...overdue.map((item) => _alertRow(Icons.schedule_rounded, '${item['title']} is overdue (${item['priority']})')),
          ],
        ],
      ),
    );
  ...
  Widget _operationSummary() {
    ...
    return _panel(
      'Tasks by Operation',
      entries.isEmpty
          ? const [Text('No task data yet.', style: TextStyle(color: AppColors.textSecondary))]
          : entries
              .take(8)
              .map(
                (entry) => Padding(
  ...
  Widget _recentActivity() {
    ...
    for (final raw in jobs.take(4)) {
      ...
      children.add(_activityRow(Icons.add_task_rounded, 'New job: ${job['title']}'));
    }
    return _panel(
      'Recent Activity',
      children.isEmpty
