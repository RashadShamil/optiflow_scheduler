...
class _ScheduleScreenState extends State<ScheduleScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _resources = List<Map<String, dynamic>>.from(payload['resources'] as List? ?? const []);
          _tasks = List<Map<String, dynamic>>.from(payload['tasks'] as List? ?? const []);
          _bookings = List<Map<String, dynamic>>.from(payload['bookings'] as List? ?? const []);
          _shop = Map<String, dynamic>.from(payload['shop'] as Map? ?? const {});
          _loading = false;
        });
      }
    } catch (e) {
  ...
  bool _sameDay(DateTime value) =>
      value.year == _selectedDate.year && value.month == _selectedDate.month && value.day == _selectedDate.day;
  ...
  List<Map<String, dynamic>> _bookingsForResource(String resourceId) {
    return _bookings.where((booking) {
      ...
      return booking['machine_id']?.toString() == resourceId && start != null && _sameDay(start);
    }).toList();
  }
  ...
  Future<void> _moveTask({
    ...
    final snapped = ((rawMinutes / 15).round() * 15).clamp(0, _dayMinutes - 15).toInt();
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Schedule moved and locked for future optimizer runs.')),
      );
      await _load();
    } catch (e) {
      if (mounted) {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text(locked ? 'Locked: future optimization will not move this task.' : 'Unlocked: future optimization may rearrange this task.'),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Close')),
          if (locked && task['status'] == 'SCHEDULED')
            ElevatedButton(onPressed: () => Navigator.pop(context, true), child: const Text('Unlock for Optimizer')),
        ],
      ),
    );
    ...
    return ListView(
      ...
      children: [
        Row(
          ...
          children: [
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text('Schedule', style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900)),
                Text(
        ...
        const Wrap(
          spacing: 18,
          children: [
            Text('Tip: drag a scheduled block. The backend rejects invalid capability/conflict/DAG moves.'),
  ...
  Widget _timeHeader() {
    return Row(
      children: [
        const SizedBox(width: 210, child: Padding(padding: EdgeInsets.all(16), child: Text('RESOURCE', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800)))),
        SizedBox(
            ...
            children: List.generate(_closeHour - _openHour + 1, (index) {
              ...
              return SizedBox(
                ...
                child: Align(
                  ...
                  child: Text(DateFormat('h a').format(DateTime(2026, 1, 1, hour))),
                ),
              );
            }),
          ),
  ...
  Widget _resourceRow(Map<String, dynamic> resource) {
    ...
    final bookings = resource['type'] == 'MACHINE' ? _bookingsForResource(resourceId) : <Map<String, dynamic>>[];

    return Container(
      height: 76,
      decoration: BoxDecoration(border: Border(top: BorderSide(color: AppColors.surfaceLight.withValues(alpha: 0.5)))),
      child: Row(
        children: [
          SizedBox(
            width: 210,
            child: Padding(
              ...
              child: Row(
                children: [
                  Icon(resource['type'] == 'MACHINE' ? Icons.precision_manufacturing_rounded : Icons.person_rounded, size: 18),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(resource['name']?.toString() ?? 'Resource', overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700)),
                        Text('${resource['type']} · ${resource['status']}', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                      ],
                    ),
                  ),
                ],
          ...
          DragTarget<Map<String, dynamic>>(
            ...
            onAcceptWithDetails: (details) {
              ...
              _moveTask(task: details.data, resource: resource, localDx: local.dx);
            },
            builder: (_, candidate, __) => Container(
              ...
              height: 76,
              decoration: BoxDecoration(
                color: candidate.isNotEmpty ? AppColors.primary.withValues(alpha: 0.06) : null,
              ),
              child: Stack(
                children: [
                  _gridLines(),
  ...
  Widget _gridLines() {
    return Row(
      children: List.generate(
        ...
        (_) => Container(
          width: 105,
          decoration: BoxDecoration(border: Border(left: BorderSide(color: AppColors.surfaceLight.withValues(alpha: 0.35)))),
        ),
      ),
    );
  }
  ...
  Widget _taskBlock(Map<String, dynamic> task, String resourceId) {
    ...
    final left = (startMinute / _dayMinutes * _timelineWidth).clamp(0.0, _timelineWidth).toDouble();
    final availableWidth = (_timelineWidth - left).clamp(1.0, _timelineWidth).toDouble();
    final width = (duration / _dayMinutes * _timelineWidth).clamp(1.0, availableWidth).toDouble();
    ...
    final block = GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: task['schedule_locked'] == true ? Border.all(color: Colors.white70) : null,
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Text(task['name']?.toString() ?? 'Task', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w800)),
            Text('$status${task['schedule_locked'] == true ? ' · 🔒' : ''}', maxLines: 1, style: const TextStyle(color: Colors.white70, fontSize: 9)),
          ],
        ),
      ),
    );
    ...
    return Positioned(
      ...
      child: movable
          ? LongPressDraggable<Map<String, dynamic>>(
              data: task,
              feedback: Material(color: Colors.transparent, child: Opacity(opacity: 0.9, child: block)),
  ...
  Widget _bookingBlock(Map<String, dynamic> booking) {
    ...
    return Positioned(
      ...
      child: Container(
        ...
        alignment: Alignment.centerLeft,
        decoration: BoxDecoration(color: Colors.blueGrey, borderRadius: BorderRadius.circular(7)),
        child: Text(
