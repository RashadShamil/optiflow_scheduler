...
class _CapabilitiesScreenState extends State<CapabilitiesScreen> {
  ...
  Future<void> _addCapability() async {
            ...
            child: Column(
              ...
              children: [
                DropdownButtonFormField<String>(
                  ...
                  items: _resources
                      .map((item) => DropdownMenuItem(value: item['id'].toString(), child: Text(item['name'].toString())))
                      .toList(),
                  onChanged: (value) {
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _operations
                      ...
                      .toList(),
                  onChanged: (value) {
                    if (value != null) setDialogState(() => operationId = value);
                  },
                ),
                const SizedBox(height: 12),
                TextField(controller: rate, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Processing rate / hour')),
                const SizedBox(height: 12),
                TextField(controller: setup, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Setup time (minutes)')),
                const SizedBox(height: 12),
                TextField(controller: cost, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Cost / hour')),
              ],
            ),
          ),
          actions: [
    ...
    if (parsedRate == null || parsedRate <= 0 || parsedSetup == null || parsedSetup < 0 || parsedCost == null || parsedCost < 0) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter valid capability values.')));
      }
      return;
    }
        ...
        Row(
          ...
          children: [
            const Column(
              ...
              children: [
                Text('Skills Matrix', style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: AppColors.textPrimary)),
                Text('Tell the optimizer which resources can perform each operation', style: TextStyle(color: AppColors.textSecondary)),
              ],
            ),
            ElevatedButton.icon(onPressed: _addCapability, icon: const Icon(Icons.add), label: const Text('Add Capability')),
          ],
        ),
        const SizedBox(height: 24),
        if (_capabilities.isEmpty)
          const Text('No capabilities configured.', style: TextStyle(color: AppColors.textSecondary))
        else
          Card(
              ...
              child: DataTable(
                ...
                rows: _capabilities.map((capability) {
                  ...
                  return DataRow(cells: [
                    ...
                    DataCell(Text('${capability['processing_rate_per_hr'] ?? '—'}')),
                    DataCell(Text('${capability['setup_time_minutes'] ?? 0} min')),
